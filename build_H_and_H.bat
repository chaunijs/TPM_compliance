@echo off
REM ============================================================
REM  H_and_H Compliance Build - v5 (pyproject.toml based)
REM ============================================================
setlocal enabledelayedexpansion
cd /d "%~dp0"

set "LOG_FILE=%~dp0build_log_h_and_h.txt"
...
if !BUILD_RESULT! equ 0 (
    echo  BUILD SUCCESS!
    if exist "%~dp0dist\H_and_H_Compliance.exe" (
        echo  Output: %~dp0dist\H_and_H_Compliance.exe
        for %%A in ("%~dp0dist\H_and_H_Compliance.exe") do (
    ...
    echo  BUILD FAILED!
    echo  Check build_log_h_and_h.txt for details
)
echo ============================================================
echo.
echo Opening build_log_h_and_h.txt in Notepad...
timeout /t 2 >nul
...
:main
echo ============================================================
echo  H_and_H Compliance Build - v5 (pyproject.toml based)
...
if not exist "%~dp0tpm_compliance_h_and_h.py" (
    echo [ERROR] tpm_compliance_h_and_h.py not found!
    exit /b 1
)
...
if exist "%~dp0H_and_H_Compliance.spec" del /q "%~dp0H_and_H_Compliance.spec"
echo   Done
echo.
